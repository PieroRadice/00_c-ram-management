...
char *stringaUtente(char *descrizione)
{
    ...
    scanf("%99[^\n]", input);
    while (getchar() != '\n');
    char *stringa = malloc((length(input)+1) * sizeof(char));
    int i = 0;
...
int main()
{
    ...
    printf("siamo nel main dopo aver inserito il cognome\n");
    printf("Ora stampiamo nome e cognome\n");
    printf("%s %s\n",nome,cognome);
    printf("CI SIAMO RIUSCITI UTILIZZANDO LO HEAP\n");
    //è però necessario deallocare la memoria heap utilizzata prima della coclusione del programma in modo che torni a disposizione del sistema operativo.
